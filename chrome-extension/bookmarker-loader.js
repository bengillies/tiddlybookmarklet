...
var isExtension = chrome && !!chrome.extension;
...
window.Bookmarker = {
	...
	loadScript: function(url, testFn, callback) {
		...
		function _loadFromChrome() {
			chrome.extension.sendMessage({ load: url }, _loaded)
		}

		if (!testFn()) {
			if (isExtension) { _loadFromChrome() } else { _loadDirectly() }
		} else {
	...
	loadDependencies: function(callback) {
		var self = this,
			...
			jQueryLib = isExtension ? 'jquery.min.js' :
				'http://ajax.googleapis.com/ajax/libs/jquery/1.6.4/jquery.min.js',
			chrjsLib = isExtension ? 'chrjs.js' :
				'http://tiddlyspace.com/bags/tiddlyspace/tiddlers/chrjs';
