...
(function() {
	...
	// are we in the chrome or firefox extensions
	var isChromeExtension = window.chrome && !!chrome.extension,
		isFirefoxExtension = self && 'port' in self,
		isExtension = isChromeExtension || isFirefoxExtension;
	...
	if (!isExtension) {
		...
		chrome.extension.getBackgroundPage().sendDataTo(receiveMessage);
	} else if (isFirefoxExtension) {
		self.port.on('bookmark-data', function(data) {
			receiveMessage({ data: JSON.stringify(data) });
		});
	}
		...
		details.when('tiddler', function(tiddler) {
			...
			if (isFirefoxExtension) {
				function result(res) {
					...
					self.port.removeListener('save', result);
				}

				tidJSON = {
					title: tiddler.title,
					tiddler: tiddler.baseData(),
					bag: tiddler.bag.name
				};

				self.port.on('save', result);
				self.port.emit('save', tidJSON);
			} else {
...
$(function() {
	...
	if (!isExtension) {
		...
		chrome.extension.sendMessage({ message: 'load script' });
	}
